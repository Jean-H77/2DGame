...
public class Zombie extends Enemy {
    ...
    public Zombie(Vector2 pos,Player player) {
        super(pos);
        health = 10;

        runSheet = new SpriteSheet(new Texture("gameOne/zombie_walk.png"),1,8);
        runSheet.setPlay(0, 7, 0.15f, true);

        attackSheet = new SpriteSheet(new Texture("gameOne/zombie_attack.png"),1,5);
    ...
    public void update() {
        ...
        if(sheet == attackSheet){
            ...
            else if(!attack && sheet.current >= 2 && Vector2.dst(pos.x,pos.y,player.getPos().x,player.getPos().y) < 60){
                player.hit(1);
                attack = true;
        ...
        if(Vector2.dst(pos.x,pos.y,player.getPos().x,player.getPos().y) < 60){
            sheet = attackSheet;
            attackSheet.setPlay(0, 4, 0.1f, true);
            attack = false;
        }
