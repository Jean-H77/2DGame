...
public class MummyProjectile extends Enemy {
    SpriteSheet sheet;
    ...
    public MummyProjectile(Vector2 pos, Player player) {
        super(pos);
        health = 1;
        speed = 14;

        runSheet = new SpriteSheet(new Texture("gameOne/mummyProjectile.png"),1,1);
...
//        attackSheet.flip(flipX);
    ...
    public void update() {
        ...
        if(Vector2.dst(pos.x,pos.y,player.getPos().x ,player.getPos().y + 40) < 30){
            player.hit(1);
            attack = true;
